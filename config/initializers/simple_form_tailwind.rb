...
SimpleForm.setup do |config|
  ...
  config.label_class = 'text-sm font-medium text-slate-200'
  ...
  config.wrappers :vertical_form, tag: 'div', class: 'mb-4' do |b|
    ...
    b.use :input, class: "px-3 py-3 placeholder-slate-400 text-slate-200 relative bg-slate-900 rounded text-sm border-0 shadow outline-none focus:outline-none focus:ring w-full"
  ...
  config.wrappers :vertical_collection, item_wrapper_class: 'flex items-center', item_label_class: 'my-1 ml-3 block text-sm font-medium text-slate-200', tag: 'div', class: 'my-4' do |b|
    b.use :html5
    b.optional :readonly
    b.wrapper :legend_tag, tag: 'legend', class: 'text-sm font-medium text-slate-200', error_class: 'text-red-500' do |ba|
      ba.use :label_text
    end
  ...
  config.wrappers :vertical_multi_select, tag: 'div', class: 'my-4', error_class: 'f', valid_class: '' do |b|
    ...
    b.wrapper tag: 'div', class: 'inline-flex space-x-1' do |ba|
      ...
      ba.use :input, class: 'flex w-auto w-auto shadow appearance-none border border-gray-300 rounded w-96 p-2 focus:outline-none focus:border-blue-500 text-slate-200 leading-4 transition-colors duration-200 ease-in-out'
    end
    b.use :full_error, wrap_with: { tag: 'p', class: 'mt-2 text-red-500 text-xs italic' }
    b.use :hint, wrap_with: { tag: 'p', class: 'mt-2 text-slate-200 text-xs italic' }
  end
